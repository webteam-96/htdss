...
export default async function PostDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <section className="container-page py-10 sm:py-14 lg:py-16">
      {post.date && <p className="text-sm font-semibold text-teal-700 mb-3 sm:mb-4">{formatDate(post.date)}</p>}
      {post.excerpt && <p className="text-base sm:text-lg text-ink-600 leading-relaxed max-w-3xl mb-6 sm:mb-8">{post.excerpt}</p>}
